...
void test_func()
{
    ...
    for (int i = 0; i < 30; i++) {
        char buff[1024];
        sprintf_s(buff, 1023, "insert into test1 values (%d, 'hello world, %d', %d);", 10, i, 100);
    ...
    for (int i = 0; i < 30; i++) {
        char buff[1024];
        sprintf_s(buff, 1023, "insert into test1 values (%d, 'hello world, %d', %d);", 20, i, 200);
        ret = do_dml_command(db, buff);
        EXPECT_TRUE(ret == SQLITE_DONE);
    }
    ...
    for (int i = 0; i < 30; i++) {
        char buff[1024];
        sprintf_s(buff, 1023, "insert into test1 values (%d, 'hello world, %d', %d);", 300, i, i);
        ret = do_dml_command(db, buff);
        EXPECT_TRUE(ret == SQLITE_DONE);
    }
    ...
    const char sql[] = "select f1, f3, count(*), count(f2), sum(f3), min(f3), max(f3), sum(f3) + 100, sum(f3) + 100.0, (sum(f3) + 100) / count(f3) , avg(f3) from test1 where f1 > 2 group by f1";
